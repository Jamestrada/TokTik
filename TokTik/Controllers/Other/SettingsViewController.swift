                ...
                options: [
                    SettingsOption(title: "Rate App", handler: {
                        DispatchQueue.main.async {
                           ...
                           // UIApplication.shared.open(URL(string: "url to store")!, options: [:], completionHandler: nil)
                        }
                    }),
            ...
            SettingsSection(
                ...
                options: [
                    ...
                    SettingsOption(title: "Privacy Policy", handler: { [weak self] in
                        DispatchQueue.main.async {
                            guard let url = URL(string: "https://www.tiktok.com/legal/privacy-policy?lang=en") else {
                                return
                            }
                            let vc = SFSafariViewController(url: url)
                            self?.present(vc, animated: true)
            ...
            DispatchQueue.main.async {
                AuthManager.shared.signOut { success in
                    if success {
                        ...
                        navVC.modalPresentationStyle = .fullScreen // without this the user can swipe away the vc
